...
export default async function DestinationsPage({ searchParams }: SearchParams) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      {/* 標題 + 搜尋列 */}
      <div className="mb-8">
        <h1 className="font-bold text-4xl text-gray-900 mb-6">探索目的地</h1>
        <Suspense>
      ...
      {destinations.length === 0 ? (
        <div className="py-24 text-center text-gray-500">
          ...
          <p className="text-lg">找不到符合條件的目的地</p>
          <p className="text-sm mt-1">試試看其他關鍵字或分類</p>
        </div>
      ) : (
        ...
        <div className="mt-12 flex items-center justify-center gap-2">
          {page > 1 && (
            <Button asChild variant="outline" size="sm">
              <Link
                href={`/destinations?${buildParams({ category, search, page: page - 1 })}`}
              >
                上一頁
              </Link>
            </Button>
